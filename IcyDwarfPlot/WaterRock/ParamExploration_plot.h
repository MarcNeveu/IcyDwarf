...
int PlotNumChem(int PT, int nparam, int ntemp, double Tmin, double Tstep, int npressure, double Pmin, double Pstep, int npH, double pHmin,
		double pHstep, int npe, double pemin, double pestep, int nWR, double WRmin, double WRstep,
...
int UpdateDisplaysParamExploration (SDL_Renderer* renderer, SDL_Texture* background_tex, SDL_Texture* pies_tex, int nleg,
		SDL_Texture* leg_tex[nleg], int nparam, SDL_Texture* Numbers[nparam], char* FontFile, int nspecies, int itopic, int PT,
...
int Angles (int itopic, SDL_Surface **pies, char *FontFile, int ntemp, int npressure, int npH, int npe, int nWR, int temp,
		int pressure, int pH, int pe, int WR, double pie_radius, double **simdata, int *nspecies, int nleg, SDL_Texture *(*leg_tex)[nleg],
...
int ParamExploration_plot (char path[1024],	int warnings, int msgout, SDL_Renderer* renderer, int* view, int* quit, char* FontFile,
	...
	SDL_Texture* Numbers[ntemp+npressure+npH+npe+nWR];
	...
	if (transpose_data == 1) {
		...
		for (i=0;i<nsim;i++) free (datasim[i]);
		free (datasim);
	}
	if (breakup_file == 1) { // For use in spreadsheets with max 1024 columns
		...
		for (i=0;i<nsim;i++) free (out1[i]);
		for (i=0;i<nsim;i++) free (out2[i]);
		for (i=0;i<nsim;i++) free (out3[i]);
		free (out1);
		free (out2);
		free (out3);
		exit(0);
	...
	while (!(*quit) && (*view) == 3){
		...
		// Update displays
		PlotNumChem(PT, nparam, ntemp, Tmin, Tstep, npressure, Pmin, Pstep, npH, pHmin, pHstep, npe, pemin, pestep, nWR, WRmin, WRstep,
				&Numbers, FontFile);
	...
	for (i=0;i<ntemp+npressure+npH+npe+nWR;i++) SDL_DestroyTexture(Numbers[i]);
...
int UpdateDisplaysParamExploration (SDL_Renderer* renderer, SDL_Texture* background_tex, SDL_Texture* pies_tex, int nleg,
	...
	if (PT == 0) { // pH-pe display
		...
		for (i=0;i<npe;i++) {
			...
			renderTexture(Numbers[ntemp+npressure+npH+npe-i-1], renderer, x + dpanel, y + i*d);
			renderTexture(Numbers[ntemp+npressure+npH+npe-i-1], renderer, x + 2*dpanel, y + i*d);
		}
		x = 130; y = 23; d = dpanel;
		for (i=0;i<nWR;i++) {
