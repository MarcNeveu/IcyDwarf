...
double **calculate_pressure (double **Pressure, int NR, int NT, thermalout **thoutput) {
	...
	printf("IcyDwarf: Calculating pressures...\n");
	...
	if (dM == NULL) printf("IcyDwarf: Not enough memory to create dM[NR][NT]\n");
	for (r=0;r<NR;r++) {
		dM[r] = (float*) malloc(NT*sizeof(float));
		if (dM[r] == NULL) printf("IcyDwarf: Not enough memory to create dM[NR][NT]\n");
	}

	float **frock = (float**) malloc(NR*sizeof(float*));            // Fraction of rock in a shell
	if (frock == NULL) printf("IcyDwarf: Not enough memory to create frock[NR][NT]\n");
	for (r=0;r<NR;r++) {
		frock[r] = (float*) malloc(NT*sizeof(float));
		if (frock[r] == NULL) printf("IcyDwarf: Not enough memory to create frock[NR][NT]\n");
	}

	float **fh2os = (float**) malloc(NR*sizeof(float*));            // Fraction of H2O ice in a shell
	if (fh2os == NULL) printf("IcyDwarf: Not enough memory to create fh2os[NR][NT]\n");
	for (r=0;r<NR;r++) {
		fh2os[r] = (float*) malloc(NT*sizeof(float));
		if (fh2os[r] == NULL) printf("IcyDwarf: Not enough memory to create fh2os[NR][NT]\n");
	}

	float **fh2ol = (float**) malloc(NR*sizeof(float*));            // Fraction of liquid H2O in a shell
	if (fh2ol == NULL) printf("IcyDwarf: Not enough memory to create fh2ol[NR][NT]\n");
	for (r=0;r<NR;r++) {
		fh2ol[r] = (float*) malloc(NT*sizeof(float));
		if (fh2ol[r] == NULL) printf("IcyDwarf: Not enough memory to create fh2ol[NR][NT]\n");
	}

	float **fadhs = (float**) malloc(NR*sizeof(float*));            // Fraction of solid ammonia dihydrate in a shell
	if (fadhs == NULL) printf("IcyDwarf: Not enough memory to create fadhs[NR][NT]\n");
	for (r=0;r<NR;r++) {
		fadhs[r] = (float*) malloc(NT*sizeof(float));
		if (fadhs[r] == NULL) printf("IcyDwarf: Not enough memory to create fadhs[NR][NT]\n");
	}

	float **fnh3l = (float**) malloc(NR*sizeof(float*));            // Fraction of liquid ammonia in a shell
	if (fnh3l == NULL) printf("IcyDwarf: Not enough memory to create fnh3l[NR][NT]\n");
	for (r=0;r<NR;r++) {
		fnh3l[r] = (float*) malloc(NT*sizeof(float));
		if (fnh3l[r] == NULL) printf("IcyDwarf: Not enough memory to create fnh3l[NR][NT]\n");
	}
	...
	for (t=0;t<NT;t++) {
		for (r=0;r<NR;r++) {
			...
			fadhs[r][t] = thoutput[r][t].madhs / dM[r][t];
			fnh3l[r][t] = thoutput[r][t].mnh3l / dM[r][t];
		}
	}
	...
	for (t=0;t<NT;t++) {
		for (r=0;r<NR;r++) {
			for (j=r;j<NR-1;j++) { // Integral using trapezoidal method
				...
				dInt = (frock[j][t]*rhoRock + fh2os[j][t]*rhoH2os +
...
float *calculate_mass_liquid (float *Mliq, int NR, int NT, thermalout **thoutput) {
	...
	for (t=1;t<NT;t++) {
		Mliq[t] = 0.0;
		for (r=0;r<NR;r++) {
			Mliq[t] = Mliq[t] + thoutput[r][t].mh2ol*gram + thoutput[r][t].mnh3l*gram;
			// Mliq[t] = Mliq[t] + thoutput[r][t].mh2ol*gram;  // Just consider liquid water for now
		}
	}
	return Mliq;
}
